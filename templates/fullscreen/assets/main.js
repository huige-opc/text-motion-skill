...
(async function(){
  ...
  const AX = {
    ...
    typewriter(el, txt, dur){
      if(!el) return;
      dur = dur||Math.max(600, txt.length*80);
      const n = txt.length;
      el.textContent = '';
      const t0 = performance.now();
      function step(t){
        const p = Math.min(1,(t-t0)/dur);
        el.textContent = txt.slice(0, Math.floor(p*n));
        if(p<1) requestAnimationFrame(step);
    ...
    fadeUpBar(el, dur){
      ...
      function step(t){
        const p = Math.min(1,(t-t0)/dur), e = AX.ease(p);
        el.style.opacity = e;
        el.style.transform = 'translate(-50%,' + (10*(1-e)) + 'px)';
        if(p<1) requestAnimationFrame(step);
      }
      requestAnimationFrame(step);
    },
